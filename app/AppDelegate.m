...
- (intptr_t)boot {
    ...
    // swap_startup() below consumes that fd on every path -- kernel owns it
    // from there -- so this side never closes it.
    if (swapEnabled && prefs.shouldEnableSwapOnExternal) {
        ...
        if (bookmarkData != nil) {
            ...
            if (url != nil && [url startAccessingSecurityScopedResource]) {
                ...
                int fd = open(swapPath.UTF8String, O_RDWR | O_CREAT | O_CLOEXEC, 0600);
                if (fd >= 0) {
                    swap_set_external_fd(fd);
                    swapExternalURL = url;  // keep alive for the session
                    NSLog(@"swap: external USB fd %d for %@", fd, swapPath);
                } else {
    ...
    swap_startup();
    // swap_startup() consumed the external fd either way (held by the area,
    // or closed). If swap did not come up, release the security-scoped URL so
    // the USB drive is unmountable; if it did, hold it for the session.
    if (!swap_enabled()) {
