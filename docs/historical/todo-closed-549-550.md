# TODO entries closed in the 549 and 550 cycles

**Archived, not maintained.** Split out of `docs/TODO.md` on 2026-09-07, when
that file's closed history had grown to two thirds of its length and was
crowding out the open work it exists to carry. Nothing here is open. Nothing
here should be edited.

Kept rather than deleted for the reason the archive exists at all: each entry
records what was *believed* while the bug was live, and several of them record a
diagnosis that turned out to be wrong in a way worth being able to look up. The
`pread_stack_thread_race` pair below is the clearest case — the second entry is
the original one, retained deliberately underneath the fix that superseded it,
because the reflex it warns about ("the known flake") is what kept the bug
invisible for weeks.

Three entries here were still filed under **"Diagnosed, not fixed"** in
`docs/TODO.md` when this split was made, months after they were closed. They are
the first three sections below.

Open work lives in [docs/TODO.md](../TODO.md). What the project intends to do
next lives in [docs/roadmap.md](../roadmap.md).

---

## Closed, but still filed as open until 2026-09-07

These three sat under "Diagnosed, not fixed" long after they were fixed.
