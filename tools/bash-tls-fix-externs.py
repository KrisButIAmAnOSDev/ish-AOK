...
ALLOW = {
    ...
    # ---- dash (build/libdash.a), converted by tools/dash-tls-rewrite.py ----
    # Every one of these is read-only in practice AND is the address half of a
    # constant initializer somewhere, so converting it would force a runtime
    # fixup to isolate a value nothing ever writes. The ALLOW set in
    # dash-tls-rewrite.py is the same list; these two must agree.
    #
    # mystring.c: `char nullstr[1]`, the zero-length string. Its ADDRESS is the
    # value -- jobs.c stores it in ps->cmd to mean "no text" and nextopt() is
    # called with it to mean "no options". Never written.
    "nullstr",
    # var.c: the initial text of IFS and OPTIND, named in varinit[]'s
    # initializer. Both are VTEXTFIXED, which is exactly the flag meaning "this
    # text is not heap and is never freed or written"; assigning to either
    # variable allocates new text instead.
    "defifsvar", "defoptindvar",
    # syntax.c: the character-class tables, generated by mksyntax. Read for
